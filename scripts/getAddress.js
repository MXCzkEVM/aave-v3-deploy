...
async function main() {
    const POOL_DATA_PROVIDER = (await deployments.get("PoolDataProvider-Aave")).address;
    const POOL_ADDRESS_PROVIDER = (await deployments.get("PoolAddressesProvider-Aave")).address;
    const ACL_MANAGER = (await deployments.get("ACLManager-Aave")).address;
    const POOL_CONFIGURATOR = (await deployments.get("PoolConfigurator-Proxy-Aave")).address;
    const POOL = (await deployments.get("Pool-Proxy-Aave")).address;
    const WrappedTokenGatewayV3 = (await deployments.get("WrappedTokenGatewayV3")).address;
    const UiIncentiveDataProviderV3 = (await deployments.get("UiIncentiveDataProviderV3")).address;
    const UiPoolDataProviderV3 = (await deployments.get("UiPoolDataProviderV3")).address;
    const WalletBalanceProvider = (await deployments.get("WalletBalanceProvider")).address;
    const rateStrategyStableOne = (await deployments.get("ReserveStrategy-rateStrategyStableOne")).address;
    const rateStrategyStableTwo = (await deployments.get("ReserveStrategy-rateStrategyStableTwo")).address;
    const rateStrategyVolatileOne = (await deployments.get("ReserveStrategy-rateStrategyVolatileOne")).address;
    ...
    const StableDebtToken = (await deployments.get("StableDebtToken-Aave")).address;
    const VariableDebtToken = (await deployments.get("VariableDebtToken-Aave")).address;
    ...
    const ReservesSetupHelper = (await deployments.get("ReservesSetupHelper")).address;
    ...
    console.log(`FAUCET: "${Faucet}",`);
    console.log("---------interface--------------------");
    const TOKEN_DAI = (await deployments.get("DAI-TestnetMintableERC20-Aave")).address;
    const TOKEN_WETH = (await deployments.get("WETH-TestnetMintableERC20-Aave")).address;
    const TOKEN_LINK = (await deployments.get("LINK-TestnetMintableERC20-Aave")).address;
    const TOKEN_USDC = (await deployments.get("USDC-TestnetMintableERC20-Aave")).address;
    const TOKEN_WBTC = (await deployments.get("WBTC-TestnetMintableERC20-Aave")).address;
    const TOKEN_USDT = (await deployments.get("USDT-TestnetMintableERC20-Aave")).address;
    const TOKEN_AAVE = (await deployments.get("AAVE-TestnetMintableERC20-Aave")).address;
    const TOKEN_EURS = (await deployments.get("EURS-TestnetMintableERC20-Aave")).address;
